...
function FormularioPost() {
  ...
  const enviarFormulario = async (navigateAfter = true) => {
    try {
      setCargando(true);
      ...
      if (isEditing && !navigateAfter) {
        // Quedarse en la edición: actualizar el formulario con la respuesta
        setFormData({
          title: respuesta.data.title,
          body: respuesta.data.body,
          userId: respuesta.data.userId
  ...
  const handleUpdate = (goToList) => {
    enviarFormulario(goToList);
  };
